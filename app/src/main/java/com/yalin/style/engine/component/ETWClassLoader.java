...
public class ETWClassLoader extends DexClassLoader {
    ...
    public String findLibrary(String name) {
        ...
        String soName = NativeFileHelper.getNativeFileName(dexPath, name);
        File targetFile = new File(NativeFileHelper.getNativeDir(context), soName);
    ...
    private boolean maybeCopyNativeLib(String libName) {
        try {
            File soFile = new File(NativeFileHelper.getNativeDir(context).getAbsolutePath()
                    + File.separator + NativeFileHelper.getNativeFileName(dexPath, libName));
            if (soFile.exists()) {
                if (soFile.length() > 0) {
                    return true;
                } else {
